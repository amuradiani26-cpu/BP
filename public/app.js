  ...
  try {
    ...
    if (tariffsGrid) {
      if (packages.length > 0) {
        tariffsGrid.innerHTML = packages.map(pkg => `
          <div class="card tariff-card ${pkg.popular ? 'popular' : ''}">
            ${pkg.popular ? '<span class="pill">პოპულარული</span>' : ''}
            <div class="tariff-header">
              <span class="tariff-name">${pkg.plan}</span>
              <span class="tariff-price">${pkg.price}<small>₾</small></span>
            </div>
            <p class="text-muted">${pkg.description || ''}</p>
            <p><strong>${pkg.tokens} BP</strong></p>
            <ul class="tariff-features">
              ${(pkg.features || []).map(f => `<li>✓ ${f}</li>`).join('')}
            </ul>
            <div class="tariff-actions">
              <button class="btn btn-primary btn-block" onclick="selectTariff('${pkg.plan}', ${pkg.price})">
                არჩევა
              </button>
            </div>
          </div>
