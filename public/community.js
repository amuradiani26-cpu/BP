// ======================================
// BEAUTY PASS COMMUNITY - საზოგადოება
// ======================================
...
const CommunityModule = {
    ...
    return `
      <div class="post-card" data-id="${post._id}">
        ...
        <div class="post-stats">
          <span>❤️ ${post.likes || post.reactionCount || 0}</span>
          <span>💬 ${post.comments || post.commentCount || 0} კომენტარი</span>
        </div>
        <div class="post-actions-bar">
          <button class="action-btn ${isLiked ? 'liked' : ''}" onclick="CommunityModule.toggleLike('${post._id}')">
            ${isLiked ? '❤️' : '🤍'} მოწონება
          </button>
          <button class="action-btn" onclick="CommunityModule.toggleInlineComments('${post._id}')">
            💬 კომენტარი
          </button>
          <button class="action-btn" onclick="CommunityModule.sharePost('${post._id}')">
            📤 გაზიარება
          </button>
        </div>
        <!-- Inline comments section -->
        <div class="inline-comments" id="comments-${post._id}" style="display: none; margin-top: 12px; border-top: 1px solid #f0f0f0; padding-top: 12px;">
          <div class="comments-list-inline" id="commentsList-${post._id}">
            <div style="text-align: center; padding: 10px; color: #888;">იტვირთება...</div>
          </div>
          <div class="add-comment-inline" style="display: flex; gap: 8px; margin-top: 10px;">
            <input type="text" id="commentInput-${post._id}" placeholder="კომენტარის დაწერა..." style="flex: 1; padding: 10px 14px; border: 1px solid #e5e7eb; border-radius: 20px; font-size: 14px; outline: none;" onkeypress="if(event.key==='Enter') CommunityModule.addInlineComment('${post._id}')">
            <button onclick="CommunityModule.addInlineComment('${post._id}')" style="padding: 10px 16px; background: linear-gradient(135deg, #e84393, #fd79a8); color: white; border: none; border-radius: 20px; cursor: pointer; font-size: 14px;">➤</button>
          </div>
        </div>
      </div>
    `;
  ...
  toggleLike: async function(postId) {
    try {
      ...
      if (postCard) {
        ...
        const likeStat = postCard.querySelector('.post-stats span:first-child');
        
        if (data.liked) {
          likeBtn.classList.add('liked');
          likeBtn.innerHTML = '❤️ მოწონება';
        } else {
          likeBtn.classList.remove('liked');
          likeBtn.innerHTML = '🤍 მოწონება';
        }
        
        if (likeStat) {
          likeStat.textContent = `❤️ ${data.likes}`;
        }
      }
        ...
        <div class="comment-inline-item" style="display: flex; gap: 10px; padding: 10px 0; border-bottom: 1px solid #f5f5f5;">
          ...
          <div style="flex: 1; min-width: 0;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
              <span style="font-weight: 600; font-size: 13px; color: #333;">${comment.author?.firstName || 'მომხმარებელი'}</span>
              <span style="color: #9ca3af; font-size: 11px;">${this.timeAgo(comment.createdAt)}</span>
            </div>
            <p style="margin: 0; font-size: 14px; color: #444; word-break: break-word;">${comment.content}</p>
          </div>
        </div>
      `).join('');
  ...
  addInlineComment: async function(postId) {
    ...
    if (!content) {
      showToast('ჩაწერეთ კომენტარი', 'error');
      return;
    }
    ...
    try {
      ...
      const postCard = document.querySelector(`.post-card[data-id="${postId}"]`);
      if (postCard) {
        const statSpan = postCard.querySelector('.post-stats span:nth-child(2)');
        if (statSpan) {
          const currentCount = parseInt(statSpan.textContent.match(/\d+/) || 0);
          statSpan.textContent = `💬 ${currentCount + 1} კომენტარი`;
        }
      }
